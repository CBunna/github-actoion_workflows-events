...
It's written for someone who is just starting out — no prior CI/CD experience needed.
...
In this project that's `.github/workflows/demo1.yaml` and `.github/workflows/demo-filters.yaml`.
...
on:
  push:
    branches:
      - main
      - "dev-*"     # wildcard: matches dev-01, dev-22, etc.
      - "feat/**"   # matches feat/anything, feat/nested/anything
```

Only pushes to `main`, or a `dev-*`/`feat/**` branch, will trigger this.
...
Or the inverse — run for everything *except* certain paths:

```yaml
on:
  push:
    paths-ignore:
      - ".github/workflows/*"   # don't re-trigger CI just for workflow edits
```
...
This mirrors the `demo-filters.yaml` workflow in this repo:

```yaml
name: Practical Events 1
on:
  pull_request:
    types: ["opened"]
  workflow_dispatch:
...
> "Run this job when a pull request is first opened, OR someone manually
> clicks 'Run workflow', OR someone pushes to `main`/`dev-*`/`feat/**` —
> unless the only thing that changed was a workflow file."
...
### 8.1 ⚠️ Common Trap: Dumping `toJson(github.event)` straight into `run:`
...
This looks harmless but breaks in a confusing way. `${{ ... }}` expressions are
substituted as **raw text before the shell ever runs** — so the entire
multi-line JSON payload gets pasted straight into your `echo "..."` line. The
JSON contains its own `"` characters, which close your quote early. Then the
shell tries to *execute* the leftover JSON lines as if they were commands,
producing errors like:

```
/home/runner/work/_temp/xxxx.sh: line 74: timestamp:: command not found
```

**Fix:** pass the value through an environment variable instead. Env vars are
set directly, without the shell re-parsing the text as a script:
...
**Rule of thumb:** any time you want to use an `${{ expression }}` inside a
`run:` step, prefer routing it through `env:` first. It avoids quoting bugs
and — more importantly — prevents shell/command injection if the value ever
comes from something an outside user controls (like a PR title or issue body).
...
4. Use `github.event_name` / `github.event.*` inside steps if your job needs to
   know *why* it was triggered — and always pass expression values through
   `env:` rather than pasting them straight into `run:`.

---

## 10. Where to Practice in This Repo

Two workflow files exist for hands-on practice:

```
.github/workflows/demo1.yaml
.github/workflows/demo-filters.yaml
```

Try editing the `on:` sections with the patterns above, commit, and watch the
